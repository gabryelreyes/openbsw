...
// DoCanTransportLayer with ISO 15765-2 extended addressing (EA) for the
// special (but common) case where a contiguous range of CAN identifiers maps
// arithmetically onto a contiguous range of transport addresses, i.e.
// transportId = canId - baseCanId (see DoCanRangeExtendedAddressingFilter).
// This example maps the range of CAN identifiers 0x700-0x7FF onto the
// transport addresses 0x00-0xFF, so the tester (address 0xF1) transmits on
// 0x7F1 and the ECU (address 0x2A, the address used for the ECU in
// referenceApp) transmits on 0x72A - the very same CAN identifiers as in
// DoCanReadVinIntegrationTestEA.cpp, but derived arithmetically here instead
// of being configured individually per participant, and without requiring an
// explicit lookup table.
//
// As with the other integration tests in this directory, it exercises a
...
// base of the range of CAN identifiers 0x700-0x7FF, mapped onto transport addresses 0x00-0xFF.
constexpr uint32_t BASE_CAN_ID = 0x700U;

// N_SA of the tester (CAN identifier 0x700 + 0xF1 = 0x7F1).
constexpr uint16_t TESTER_ID = 0xF1U;
// N_TA of the ECU (CAN identifier 0x700 + 0x2A = 0x72A; also the address used for the ECU in
// referenceApp).
constexpr uint16_t ECU_ID    = 0x2AU;

constexpr uint32_t REQUEST_CAN_ID  = BASE_CAN_ID + TESTER_ID;
constexpr uint32_t RESPONSE_CAN_ID = BASE_CAN_ID + ECU_ID;
...
TEST(DoCanReadVinIntegrationTestEARange, SingleFrameRequestYieldsSegmentedResponse)
{
    ...
    // EXAMPLE_START DoCanRangeExtendedAddressingFilter
    // the entire range of CAN identifiers 0x700-0x7FF maps arithmetically onto transport
    // addresses 0x00-0xFF (transportId = canId - baseCanId), so no explicit per-participant table
    // is needed - any of the 256 possible addresses can address any other.
    DoCanRangeExtendedAddressingFilter<DataLinkLayer> addressingFilter{
        BASE_CAN_ID, 0x00U, 0x100U, {}, codec};
    // EXAMPLE_END DoCanRangeExtendedAddressingFilter
