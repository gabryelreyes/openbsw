...
public:
     ...
     *
     * On the S32K344 receive and transmit message buffers share one interrupt
     * line, so the shared handler uses this to decide whether to run the
     * transmit ISR.
     */
    static bool transmitInterruptPending(uint8_t const transceiverIndex)
    {
        return fpCanTransceivers[transceiverIndex]->fFlexCANDevice.isTransmitInterruptPending();
    }
