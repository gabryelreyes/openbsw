...
, fBusOff(false)
{
    ...
    {
        case IP_CAN_0_BASE: fIndex = 0; break;
        case IP_CAN_1_BASE: fIndex = 1; break;
        case IP_CAN_2_BASE: fIndex = 2; break;
...
{
    // Pin multiplexing of the CAN pads (SIUL2, replaces the S32K1xx Io
    // configuration).
    IP_SIUL2->MSCR[fConfig.txPadMscr]
        = SIUL2_MSCR_OBE_MASK | static_cast<uint32_t>(fConfig.txPadSss);
    IP_SIUL2->MSCR[fConfig.rxPadMscr] = SIUL2_MSCR_IBE_MASK;
    IP_SIUL2->IMCR[fConfig.rxImcr]    = SIUL2_IMCR_SSS(fConfig.rxImcrSss);
    ...
    // The S32K344 message buffer RAM and the individual RX mask registers are
    // ECC protected and are not initialized by the soft reset. Write all of
    // them once so that accesses (also by the CAN engine, which scans all
    // mailboxes up to MAXMB) do not raise ECC errors.
    {
        vuint32_t* const messageBufferRam
            = reinterpret_cast<vuint32_t*>(fConfig.baseAddress + MESSAGE_BUFFER_RAM_OFFSET);
        for (uint32_t i = 0U; i < MESSAGE_BUFFER_RAM_WORDS; ++i)
        {
            messageBufferRam[i] = 0U;
        }
        for (uint32_t i = 0U; i < FLEXCAN_RXIMR_COUNT; ++i)
        {
            fpDevice->RXIMR[i] = 0U;
        }
    }
