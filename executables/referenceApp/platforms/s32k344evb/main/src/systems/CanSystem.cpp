...
{
    ::asyncEnterIsrGroup(ISR_GROUP_CAN);
    // RX and TX message buffers share one interrupt line on the S32K344:
    // only run the transmit ISR if its interrupt is actually pending.
    if (bios::CanFlex2Transceiver::transmitInterruptPending(0))
    {
